  ...
  useEffect(() => {
    const fetchCases = async () => {
      try {
        ...
        const casesList = caseSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Case));
        setCases(casesList);

        // Calculate stats
        const active = casesList.filter(c => c.status === "In Hearing" || c.status === "Under Review").length;
        const underReview = casesList.filter(c => c.status === "Under Review").length;
        const resolved = casesList.filter(c => c.status === "Ruling" || c.status === "Closed").length;
